...
parser.add_argument('--model-dir', default='./model-mnist-smallCNN/TRADES',
...
def train(args, model, device, train_loader, optimizer, epoch, logger):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        loss, loss_natural, loss_robust = trades_loss(model=model,
                           x_natural=data,
                           y=target,
                           optimizer=optimizer,
        ...
        losses_natural.update(loss_natural, data.size(0))
        losses_robust.update(loss_robust, data.size(0))
        # print progress
    ...
    logger.log_value('train_natural loss', losses_natural.avg, epoch)
    logger.log_value('train_robust loss', losses_robust.avg, epoch)
...
def main():
    # settings
    model_dir = os.path.join(args.model_dir, 'trades_beta-{}'.format(args.beta))
