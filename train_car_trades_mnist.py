...
from car_trades import car_trades_loss
from utils import AverageMeter
from crd.criterion import CRDLoss


parser = argparse.ArgumentParser(description='PyTorch MNIST CAR TRADES Adversarial Training')
...
parser.add_argument('--model-dir', default='./model-mnist-smallCNN/CAR-TRADES',
...
def train(args, model, device, train_loader, optimizer, criterion_car, epoch, logger):
    ...
    for batch_idx, (data, target, index, contrast_index) in enumerate(train_loader):
        ...
        loss, loss_natural, loss_robust, loss_car = car_trades_loss(model=model,
                              x_natural=data,
                              y=target,
                              optimizer=optimizer,
                              car_beta=args.car_beta,
                              trades_beta=args.beta,
                              idx=index,
        ...
        losses_natural.update(loss_natural.item(), data.size(0))
    ...
    logger.log_value('loss natural', losses_natural.avg, epoch)
...
def main():
    # settings
    model_dir = os.path.join(args.model_dir, 'car_beta-{}-trades_beta-{}'.format(args.car_beta, args.beta))
